...
data <- data.frame(
  x = c(2, 2.1, 2, 2.2, 1.5, 1, 0.2, 0.5, 1.4), # inotropic effect
  y = c(1.5, 1.8, 2.2, 2.5, 0, 0.5, 0.3, 0.8, 0.2), # chronotropic effect
  z = c(-1, 2, 1, -1, -2, 2, -1, 1, -1.8), # vascular effect (negative = vasodilator, positive = vasoconstrictor)
  labels = c("Dobutamine", "Epinephrine (high dose)", "Dopamine (high dose)", "Isoproterenol", "Milrinone", "Norepinephrine", "Dopamine (low dose)", "Epinephrine (low dose)", "Levosimendan")
)
...
ggplot(data, aes(x = x, y = y)) +
  ...
  coord_cartesian(xlim = c(min(data$x), max(data$x) + 1.4), 
                  ylim = c(min(data$y), max(data$y) + 0.2)) + # Further expand space for labels
  theme_minimal() +
  theme(
